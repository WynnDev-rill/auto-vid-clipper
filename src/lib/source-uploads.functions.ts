...
export const SOURCE_VIDEOS_BUCKET = "clipforge-source-videos";
...
export const prepareSourceUpload = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    if (error) throw new Error(`Could not prepare video upload: ${error.message}`);
    return { path, token: signed.token, originalName: data.fileName };
  });
