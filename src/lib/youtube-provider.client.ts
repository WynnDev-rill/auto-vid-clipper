...
export async function persistYouTubeProviderSession(session: Session) {
  ...
  // Provider access tokens normally last about one hour. Keep a safety margin so
  // generation never starts with a token that will expire halfway through.
  ...
  const { error } = await supabase.from("clipforge_youtube_connections").upsert(
    {
      ...
      // We deliberately do not persist Google's provider refresh token in the
      // browser-facing database. When the access token expires the UI asks the
      // user to reconnect, rather than storing a long-lived Google credential.
