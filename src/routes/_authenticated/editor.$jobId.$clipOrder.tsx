...
import { exportMoment, exportSubtitles, getJob, getPreferences, saveUrl, type ExportOptions } from "@/lib/clipforge-client";
...
function Editor() {
  ...
  async function downloadSrt() {
    ...
    try {
      const result = await exportSubtitles(jobId, { clipOrder: order, trimStart: options.trimStart, trimEnd: options.trimEnd });
      saveUrl(result.url, `ClipForge-Moment-${order + 1}.srt`);
      toast.success("Subtitle download started");
    } catch (error) { toast.error(error instanceof Error ? error.message : "Could not export subtitles"); }
  ...
  return <div className="mx-auto max-w-3xl space-y-5">
    ...
    <div className="grid gap-5 md:grid-cols-[320px_1fr]">
      ...
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-2 gap-2"><button onClick={() => saveUrl(clip.video_url, `ClipForge-Preview-${order + 1}.mp4`)} className="flex items-center justify-center gap-2 rounded-2xl border border-border px-3 py-3 text-xs"><Download size={14} /> Preview</button><button onClick={downloadSrt} disabled={subtitleBusy} className="flex items-center justify-center gap-2 rounded-2xl border border-border px-3 py-3 text-xs disabled:opacity-50">{subtitleBusy ? <Loader2 size={14} className="animate-spin" /> : <FileText size={14} />} SRT subtitles</button></div>
      </div>
    </div>
  </div>;
}
